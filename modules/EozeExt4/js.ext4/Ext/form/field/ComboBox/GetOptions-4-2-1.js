 ...
 */
Ext.define('Eoze.Ext.form.field.ComboBox.GetOptions-4-2-1', {
	override: 'Ext.form.field.ComboBox'

	,getLoadOptions: function(queryString, queryPlan) {
		return {
	...
	,doRemoteQuery: function(queryPlan) {
		...
		if (me.pageSize) {
			...
			me.loadPage(1, Ext.apply({
				rawQuery: queryPlan.rawQuery,
				callback: loadCallback
			}, this.getLoadOptions(queryPlan.rawQuery, queryPlan)));
		} else {
			var options = Ext.apply({
				rawQuery: queryPlan.rawQuery,
				callback: loadCallback
			}, this.getLoadOptions(queryPlan.rawQuery, queryPlan));
			me.store.lastOptions = options;
