...
Ext.define('Ext.ux.GroupTabPanel', {
	...
	initComponent: function(config) {
		...
		me.items = [{
			xtype: 'treepanel',
			cls: 'x4-tree-panel x4-grouptabbar'
				+ (this.roundedCorners ? ' ' + Ext.baseCSSPrefix + 'grouptabs-corner-rounded' : ''),
			width: 150,
			rootVisible: false,
			store: me.store,
			hideHeaders: true,
			...
			columns: [{
				...
				renderer: function (value, cell, node, idx1, idx2, store, tree) {
					...
					if (node.parentNode && node.parentNode.parentNode === null) {
						cls += ' x4-grouptab-first';
						if (node.previousSibling) {
							cls += ' x4-grouptab-prev';
						}
						if (!node.get('expanded') || node.firstChild == null) {
							cls += ' x4-grouptab-last';
						}
					} else if (node.nextSibling === null) {
						cls += ' x4-grouptab-last';
					} else {
						cls += ' x4-grouptab-center';
					}
					if (node.data.activeTab) {
						cls += ' x4-active-tab';
					}
					cell.tdCls= 'x4-grouptab'+ cls;
...
Ext.define('Ext.ux.GroupTreeChunker', {
	...
	metaRowTpl: [
		...
		'<div class="x4-grouptabs-corner x4-grouptabs-corner-top-left" id="ext-gen25"></div>',
		'<div class="x4-grouptabs-corner x4-grouptabs-corner-bottom-left" id="ext-gen26"></div>',
		'</div>',
		'</tpl>',
		'</div>'
	],
