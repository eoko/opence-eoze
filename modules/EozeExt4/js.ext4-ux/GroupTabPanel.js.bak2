 ...
 */
Ext.define('Ext.ux.GroupTabPanel', function() {

	var x = Ext.baseCSSPrefix,
		sx = ' ' + x;
	...
	return {
		...
		initComponent: function(config) {
			...
			me.items = [{
				xtype: 'treepanel',
				cls: x + 'tree-panel' + sx + 'grouptabbar',
				width: 150,
				rootVisible: false,
				store: me.store,
				hideHeaders: true,
				...
				columns: [{
					...
					renderer: function (value, cell, node, idx1, idx2, store, tree) {
						...
						if (node.parentNode && node.parentNode.parentNode === null) {
							cls += sx + 'grouptab-first';
							if (node.previousSibling) {
								cls += sx + 'grouptab-prev';
							}
							if (!node.get('expanded') || node.firstChild == null) {
								cls += sx + 'grouptab-last';
							}
						} else if (node.nextSibling === null) {
							cls += sx + 'grouptab-last';
						} else {
							cls += sx + 'grouptab-center';
						}
		...
		setActiveTab: function(cmp) {
			...
			if (me.fireEvent('beforetabchange', me, newTab, oldTab) !== false) {
				 me.activeTab = newTab;
				 if (me.rendered) {
					 me.down('container[baseCls=' + x + 'grouptabcontainer' + ']').getLayout().setActiveItem(newTab);
				 }
