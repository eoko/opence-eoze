...
//        $command .= ' --title "' . $this->getTitle() . '"';
		...
		Logger::get($this)->debug('Running command: ' . $command);
	...
	public function setHeaderHtml($headerHtml) {
		$this->options['header-html'] = $headerHtml;
	}

	public function __call($fn, $args) {
		if (substr($fn, 0, 3) === 'set') {
			$this->options[Inflector::camelCaseToUnderscored(substr($fn, 3), '-')] = $args[0];
		} else {
			throw new UnsupportedOperationException(get_class($this) . "::$fn()");
		}
	}
