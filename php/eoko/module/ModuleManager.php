...
class ModuleManager {
	...
	public static function getModule($name, $required = true) {
		...
		// save in cache dependency pile, if needed
	...
	private function doInstantiateModule($name, $required, &$cacheDeps) {
		...
		// try to delegate
		if (self::$moduleFactories) {
			foreach (self::$moduleFactories as $factory) {
	 ...
	 * the case of auto generated configuration values that the normal config
