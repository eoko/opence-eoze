...
if (ConfigManager::get('eoko/log/appenders/Output') || isset($_GET['olog'])) {
    if (isset($_SERVER['argc']) || !empty($_SERVER['argc'])) {
        $shell = new \eoko\shell\ShellEoze();
        Logger::addAppender(new LoggerOutputAppender('SHELL', $shell));
        $shell->startConsole();
    } else {
        Logger::addAppender(new LoggerOutputAppender("VOID"));
    }
}
...
if ((!isset($test) || !$test)
        ...
        && (!isset($_SERVER['argc']) || empty($_SERVER['argc']))
