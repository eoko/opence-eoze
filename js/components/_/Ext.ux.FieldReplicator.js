...
//			this.oldValue = f.getValue();
//			f.on('select', function(c,r,i){
//				if (this.oldValue !== c.getValue()) {
//					this.onChange(c,c.getValue(),this.oldValue);
//				}
//			}, this);
//		} else {
...
//              Field filled, insert a clone as the next sibling
				var conf = f.cloneConfig(), m;
				if (conf.name) {
					m = /(.*?)(\d+)/.exec(conf.name);
//					conf.name = m[1] + (parseInt(m[2]) + 1);
					conf.name = m[1] + (++f.replicatorIndex);
				} else if (conf.hiddenName) {
					m = /(.*?)(\d+)/.exec(conf.hiddenName);
					conf.hiddenName = m[1] + (++f.replicatorIndex);
				}
                ns = new f.constructor(conf);
