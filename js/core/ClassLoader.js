...
//		Oce.Logger.error(error);
	...
	function requestModule(fn, controller, jsModule, successCallback, errorCallback) {
	 ...
	 */
	function loadFile(controller, name, callback, action) {

		if (action === undefined) action = 'get_js';
...
//			Oce.Logger.debug('src=' +newScript.src);
	...
	var ControllerClassLoader = function(controller) {
		...
		var countDown = function(success, newErrors) {
			...
			if (doneCount === 0) processCallbacks();
		}
...
//					} else {
					...
					// Countdown
					if (--count == 0) {
						if (errors.length == 0) callback(true);
						else callback(false, errors);
					}
				})
...
//		},
//
//		get: function(depName) {
//			if (depName in loadedModules) {
//				return loadedModules[depName];
//			} else {
//				Oce.ClassLoader(load)
//			}
//		}
//	}
//}
